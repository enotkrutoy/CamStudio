...
// The 'aistudio' property on window is already defined as 'AIStudio' by the environment.
// Redeclaring it here with a structural type caused a TypeScript "Subsequent property declarations must have the same type" error.

const LOADING_MESSAGES = [
  "АНАЛИЗ: Сканирование геометрии объекта...",
  "VOXEL_MAP: Построение объемной сетки...",
  "RAY_CAST: Симуляция пути лучей 50мм оптики...",
  "NEURAL_DRAW: Реконструкция пиксельных кластеров...",
  "FINAL_LUT: Применение кинематографической цветокоррекции...",
  "IO_SYNC: Перенос кадра в основной буфер..."
];

const App: React.FC = () => {
  const { state: cameraState, updateState: updateCamera, generatedPrompt } = useCameraControls();
  ...
  const handleOpenKeyPicker = async () => {
    try {
      // Access pre-configured aistudio object
      if ((window as any).aistudio) {
        await (window as any).aistudio.openSelectKey();
        setShowKeyPicker(false);
        // Assume success and proceed per rules
        await startGenerationFlow();
      }
    } catch (err) {
      setError("Не удалось инициализировать защищенный выбор ключа.");
    }
  };
  ...
  const startGenerationFlow = async () => {
    ...
    if (settings.quality === 'pro') {
      try {
        ...
        console.warn("AIStudio bridge не обнаружен, продолжаем с ключом окружения.");
      }
    }
    ...
    try {
      ...
      setError(err.message || "КРИТИЧЕСКИЙ_СБОЙ: Буфер данных поврежден.");
    } finally {
    ...
    <div className="min-h-screen bg-[#020202] text-white font-sans selection:bg-blue-500/40 overflow-x-hidden antialiased">
        ...
        <div className="max-w-[1700px] mx-auto px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                 <span className="text-[9px] text-gray-500 font-mono tracking-widest font-black uppercase">Лаборатория пространственного ИИ</span>
                 <div className="w-1 h-1 rounded-full bg-blue-500/40" />
                 <span className="text-[9px] text-blue-500/60 font-mono font-bold">ВЕРСИЯ_3.5.0</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="text-right hidden sm:block">
              <p className="text-[9px] text-gray-600 uppercase font-black tracking-widest">Статус_Узла</p>
      ...
      <main className="max-w-[1700px] mx-auto p-8 grid lg:grid-cols-[320px_1fr_420px] gap-10 relative z-10">
        <aside className="space-y-8">
          ...
          <div className="bg-gray-950/50 rounded-3xl border border-white/5 p-6 space-y-5">
            <div className="flex justify-between items-center">
              <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Творческие Директивы</h3>
              <div className="w-8 h-[1px] bg-white/10" />
            </div>
            <textarea 
              value={settings.creativeContext}
              onChange={(e) => setSettings(s => ({...s, creativeContext: e.target.value}))}
              placeholder="Добавьте детали... например: 'разрешение 8k, raw фото, блики линз'"
            ...
            <div className="flex flex-wrap gap-2">
               {['Сай-фай', 'Ретро', 'Гипер-реал', 'Студия'].map(tag => (
          ...
          <div className="space-y-4">
            ...
            {sourceImage ? (
              <div className="relative aspect-video rounded-[2.5rem] overflow-hidden border border-white/5 bg-gray-950 shadow-2xl group">
                <img src={sourceImage.base64} alt="Исходник" className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity duration-700" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
                <div className="absolute bottom-8 left-8 flex items-center gap-4">
                  <button onClick={() => setSourceImage(null)} className="w-12 h-12 bg-red-500/10 border border-red-500/30 rounded-full flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-lg">
                  ...
                  <div className="font-mono">
                    ...
                    <p className="text-[8px] text-gray-500">ВЫДЕЛЕНО: {(sourceImage.size / 1024 / 1024).toFixed(2)} МБ</p>
                  </div>
                </div>
              </div>
            ) : (
        ...
        <aside className="space-y-8">
          <div className="relative aspect-square rounded-[3.5rem] bg-black border border-white/10 shadow-[0_0_100px_rgba(0,0,0,1)] flex items-center justify-center overflow-hidden group">
            {result ? (
              <div className="relative w-full h-full">
                <img src={result.imageUrl} className="w-full h-full object-contain cursor-zoom-in animate-in fade-in duration-1000" onClick={() => setIsLightboxOpen(true)} alt="Результат генерации" />
                <div className="absolute top-6 right-6 flex gap-2">
                   <button onClick={copyImageToClipboard} className="w-10 h-10 bg-black/60 backdrop-blur-md rounded-full border border-white/10 flex items-center justify-center text-white/50 hover:text-white transition-all shadow-xl" title="Копировать в буфер">
                ...
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 px-4 py-2 bg-black/60 backdrop-blur-md border border-white/10 rounded-full text-[9px] font-black uppercase tracking-widest text-white/40">Результат Синтеза</div>
              </div>
            ) : (
              <div className="text-center space-y-6 p-12 opacity-10">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.4em]">Ожидание_Сигнала</p>
              </div>
            )}
            ...
            {isGenerating && (
              <div className="absolute inset-0 bg-black/95 backdrop-blur-2xl z-50 flex flex-col items-center justify-center gap-8 px-10">
                ...
                <div className="text-center">
                  <p className="text-blue-400 font-black text-xs tracking-[0.5em] uppercase mb-3 animate-pulse">Нейро_Синтез</p>
          ...
          <div className="grid grid-cols-2 gap-4">
             <a href={result?.imageUrl} download="render.png" className={`py-4 rounded-2xl bg-white text-black font-black text-[10px] uppercase text-center tracking-widest transition-all hover:scale-[1.02] active:scale-95 shadow-xl ${!result && 'opacity-20 pointer-events-none'}`}>
                Экспорт
             </a>
          ...
          >
            <span className="relative z-10">{isGenerating ? 'В ПРОЦЕССЕ...' : 'ЗАПУСК РЕНДЕРА'}</span>
          ...
          {error && <div className="p-5 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-500 text-[10px] font-mono leading-relaxed"><span className="font-bold mr-2">ОШИБКА:</span>{error}</div>}
        </aside>
      </main>
      ...
      {showKeyPicker && (
        <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-xl flex items-center justify-center p-8 animate-in fade-in duration-300">
          <div className="max-w-md w-full bg-gray-900 border border-white/10 rounded-[3rem] p-12 text-center space-y-8 shadow-2xl">
            ...
            <div className="space-y-4">
              <h2 className="text-2xl font-black uppercase tracking-tight">Требуется Pro-движок</h2>
              <p className="text-gray-400 text-sm leading-relaxed">
                Для использования Gemini 3 Pro необходимо выбрать API-ключ проекта с включенной тарификацией.
              </p>
              <a 
      ...
      {isLightboxOpen && result && (
        <div className="fixed inset-0 z-[100] bg-black/98 backdrop-blur-3xl flex items-center justify-center p-12 cursor-zoom-out animate-in zoom-in duration-300" onClick={() => setIsLightboxOpen(false)}>
          <img src={result.imageUrl} className="max-w-full max-h-full rounded-2xl shadow-[0_0_100px_rgba(0,0,0,1)]" alt="Зум результата" />
        </div>
      )}
